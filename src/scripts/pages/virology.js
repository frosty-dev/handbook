...
export function processVirology(root) {
    const diseaseTable = root.querySelector("[id='.D0.9F.D1.80.D0.BE.D1.81.D1.82.D1.8B.D0.B5_.D0.B7.D0.B0.D0.B1.D0.BE.D0.BB.D0.B5.D0.B2.D0.B0.D0.BD.D0.B8.D1.8F'] .wikitable");
    ...
    const symptomsTable = root.querySelector("[id='.D0.A2.D0.B0.D0.B1.D0.BB.D0.B8.D1.86.D0.B0_.D0.A1.D0.B8.D0.BC.D0.BF.D1.82.D0.BE.D0.BC.D0.BE.D0.B2'] .wikitable");
    const symptoms = parseTable(symptomsTable)
        .sort((a, b) => parseInt(a["Уровень"].textContent, 10) -
    ...
    const symptomsBetterTable = makeTable(["Symptom", "Stats", "Thresholds"], symptoms);
    symptomsBetterTable.className = "symptoms-ext wikitable";
    symptomsTable.replaceWith(symptomsBetterTable);
}
export function virologyScript(root) {
    // Init fuzzy search with elements
    const diseases = Array.from(root.querySelectorAll(".disease-ext tr:not(:first-child)"));
    registerSearchEntries(diseases.map((element, id) => ({
        page: "Infections",
    ...
    const symptoms = Array.from(root.querySelectorAll(".symptoms-ext > tbody > tr:not(:first-child)"));
    registerSearchEntries(symptoms.map((element, id) => ({
        page: "Infections",
        name: element.querySelector(".disease-name").textContent.trim(),
        element,
        alignment: "center",
