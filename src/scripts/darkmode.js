...
export var ColorFmt;
(function (ColorFmt) {
    ColorFmt[ColorFmt["RGB"] = 0] = "RGB";
    ColorFmt[ColorFmt["HEX"] = 1] = "HEX";
})(ColorFmt || (ColorFmt = {}));
...
function parseColor(input) {
    ...
    if (input[0] === "#") {
        ...
        return {
            ...
            g: Math.round(parseInt(input.substr(1 + collen, collen), 16) * fact) / 256,
...
function serializeColor(col, format) {
...
export function darken(color, format) {
...
export function lighten(color, format) {
    const col = parseColor(color);
    const hsl = rgbToHsv(col);
    if (hsl.v < 0.5) {
        hsl.v = 0.8;
