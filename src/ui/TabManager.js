...
import { processHTML, bindFunctions, PAGE_VERSIONS, postProcessHTML, } from "../scripts/index";
...
async function loadPage(page, elem, useCache) {
    let html = null;
    const key = `page:${page}`;
    ...
    if (useCache) {
        try {
            const cachedPage = await cache.get(key);
            if (cachedPage) {
                // Get expected version
                const expectedVersion = page in PAGE_VERSIONS ? PAGE_VERSIONS[page] : PAGE_VERSIONS.$DEFAULT;
    ...
    if (!html) {
        ...
        // Get version to set
        const version = page in PAGE_VERSIONS ? PAGE_VERSIONS[page] : PAGE_VERSIONS.$DEFAULT;
...
export default class TabManager {
    constructor(sectionlist, tablist, tabcontent) {
        this.sections = {};
        this.sectionMap = {};
        this.loading = false;
    ...
    setLoading(value) {
        if (value) {
            ...
            spinnerContainer.appendChild(document.createTextNode("Loading wiki pages"));
        }
        else {
     ...
     */
    showSection(name) {
        const active = this.sectionListContainer.querySelector(".active");
        if (active) {
     ...
     */
    async openTab(section, page, options) {
    ...
    setActive(page) {
        ...
        const isSectionActive = this.sections[section].element.classList.contains("active");
        if (!isSectionActive) {
